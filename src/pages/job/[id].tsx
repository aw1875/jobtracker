...
// Utils
import { deleteJobApplication, getJobById, updateJobDetails } from "../../utils/api";
import { GH_LOGOUT_URI } from "../../utils/helper";

// Icons
import { MdDateRange, MdLocationPin, MdPermContactCalendar, MdCancel, MdLogout } from 'react-icons/md';
...
const JobPage: NextPage<Props> = ({ job, headers }) => {
  ...
  const handleUpdateNotes = () => {
    updateJobDetails({ _id: job._id, status: '', notes: notesRef.current?.value ?? '', headers }, (status) => {
      if (status === 200) {
        router.replace(router.asPath);
        setEditNotes(false);
      }
  ...
  useEffect(() => {
    const getImage = async () => {
      const data = await fetch(`https://autocomplete.clearbit.com/v1/companies/suggest?query=${job.company.replace(/\s/g, '')}`).then((r) => r.json())
      ...
      <div className="w-full lg:w-2/3 flex flex-col gap-y-8 bg-gray-700 text-white rounded p-4 md:p-12">
        ...
        {editStatus && (
          <div className="grid grid-cols-1 lg:flex gap-x-4 gap-y-2 select-none">
            <button className='change-status bg-gray-400' onClick={() => handleUpdateStatus("Stale")}>Stale</button>
            <button className='change-status bg-blue-400' onClick={() => handleUpdateStatus("Applied")}>Applied</button>
            <button className='change-status bg-red-500' onClick={() => handleUpdateStatus("Rejected")}>Rejected</button>
            <button className='change-status bg-orange-300' onClick={() => handleUpdateStatus("Under Review")}>Under Review</button>
            <button className='change-status bg-green-300' onClick={() => handleUpdateStatus("Offered")}>Offered</button>
            <button className='change-status bg-green-500' onClick={() => handleUpdateStatus("Accepted")}>Accepted</button>
          </div>
        )}
        ...
        {editNotes ? (
          <>
            <textarea className="h-48 p-4 text-black resize-none outline-none rounded" placeholder="Add your notes" ref={notesRef}>{job.notes}</textarea>
          ...
          <>
            ...
            <button className='button-full text-orange-900 bg-orange-300' onClick={() => setEditNotes(true)}><FaEdit /> Edit Notes</button>
          </>
        )}
      </div>
    </div >
